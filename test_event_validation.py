...
def test_our_events_clock_shift():
    ...
    norm = normalize_our_events(raw, clip_start_s=600.0)
...
def test_match_fp_and_fn():
    ours = [_ev("pass", 10.0), _ev("pass", 50.0)]
    truth = [_ev("pass", 10.5, "statsbomb"), _ev("pass", 80.0, "statsbomb")]
    r = match_type(ours, truth, "pass", tolerance_s=3.0)
...
def test_validate_events_overall():
    ...
    assert res.per_type["recovery"].fp == 1
    assert res.per_type["interception"].fn == 1
